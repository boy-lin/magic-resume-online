...
export function Dock({ children, className, ...props }: DockProps) {
  ...
  if (templateSheetIndex === -1) {
    return (
      <div
        {...props}
        className={cn(
          "flex flex-col items-center gap-4 rounded-xl bg-white/[0.7] p-2 backdrop-blur-md dark:bg-slate-800/[0.7]",
          className
        )}
      >
  ...
  return (
    <div
      {...props}
      className={cn(
        "flex flex-col items-center gap-4 rounded-xl bg-white/[0.7] p-4 backdrop-blur-md dark:bg-slate-800/[0.7]",
        className
      )}
    >
      {/* Top group */}
...
export function DockIcon({ children, className, onClick }: DockIconProps) {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.95 }}
      className={cn(
        "flex size-8 items-center justify-center rounded-sm bg-white text-neutral-700 transition-colors hover:bg-gray-100 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700",
        className
      )}
      onClick={onClick}
    >
